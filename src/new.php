...
class SystranTranslator extends RestClient implements TranslateInterface, DictionaryInterface {
   ...
   {
      $contents = $this->request(self::$dict_languages['method'], self::$dict_languages['route']);
  ...
  // The default expected encoding is utf-8. If it is not utf-8, use the 'options' paramter to specify the endocing.
   ...
   final public function translate(string $text, string $dest, $src="") : string 
   {
      ...
      static $closure = function () use ($text) : string {

           return $this->request(self::$trans['method'], self::$trans['route'], ['query' => ['input' => $text]]); 
      } 

       $this->setLanguages(strtolower($dest), strtolower($src)); 

       $contents = $closure();

       //$contents = $this->request(self::$trans['method'], self::$trans['route'], ['query' => $this->query]); 
